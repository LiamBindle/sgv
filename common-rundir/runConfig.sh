...
# resources available.
TOTAL_CORES=REPLACE_NUM_CORES
NUM_NODES=REPLACE_NUM_NODES
NUM_CORES_PER_NODE=REPLACE_CORES_PER_NODE
...
#
common_freq="7440000"          # Ignore if using multi-run monthly diag option
common_dur="7440000"           # Ignore if using multi-run monthly diag option
...
INITIAL_RESTART=initial_GEOSChem_rst.c48_benchmark.nc
...
greater_than() {
    RV=$(awk -v num1=$1 -v num2=$2 'BEGIN{ print (num1>num2) ? "T" : "" }')
    if [ -z "$RV" ]
    then
        return 1
    else
...
if greater_than ${STRETCH_FACTOR} 1.0 ; then
    sed -i "s/^.*GCHP\.STRETCH_FACTOR\s*:\s*[0-9\.\-]*/GCHP\.STRETCH_FACTOR: ${STRETCH_FACTOR}/g"   GCHP.rc
    sed -i "s/^.*GCHP\.TARGET_LON\s*:\s*[0-9\.\-]*/GCHP\.TARGET_LON: ${TARGET_LON}/g"               GCHP.rc
    sed -i "s/^.*GCHP\.TARGET_LAT\s*:\s*[0-9\.\-]*/GCHP\.TARGET_LAT: ${TARGET_LAT}/g"               GCHP.rc
    ...
    if greater_than 0.0 ${TARGET_LON}
    then
        echo "ERROR: TARGET_LON must be in [0,360)"
        exit 1
    fi
else
    sed -i "s/^.*GCHP\.STRETCH_FACTOR\s*:\s*[0-9\.\-]*/# GCHP\.STRETCH_FACTOR:/g"   GCHP.rc
    sed -i "s/^.*GCHP\.TARGET_LON\s*:\s*[0-9\.\-]*/# GCHP\.TARGET_LON:/g"           GCHP.rc
    sed -i "s/^.*GCHP\.TARGET_LAT\s*:\s*[0-9\.\-]*/# GCHP\.TARGET_LAT:/g"           GCHP.rc

    sed -i "s/^.*do_schmidt\s*=\s*[a-zA-Z\.]*/! do_schmidt =/g"                     input.nml
    sed -i "s/^.*stretch_fac\s*=\s*[0-9\.\-]*/! stretch_fac =/g"                    input.nml
    sed -i "s/^.*target_lat\s*=\s*[0-9\.\-]*/! target_lat =/g"                      input.nml
    sed -i "s/^.*target_lon\s*=\s*[0-9\.\-]*/! target_lon =/g"                      input.nml
fi
